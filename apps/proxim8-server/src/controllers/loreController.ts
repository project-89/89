...
export const getLoreByNftId = async (
  ...
  try {
    ...
    const lore = await Lore.find({ nftId });
...
export const getClaimedLoreByNftId = async (
  ...
  try {
    const { nftId } = req.params;
    const claimedLore = await Lore.find({ nftId, claimed: true });
...
export const getAvailableLoreByNftId = async (
  ...
  try {
    const { nftId } = req.params;
    const unclaimedCount = await Lore.countDocuments({ nftId, claimed: false });
