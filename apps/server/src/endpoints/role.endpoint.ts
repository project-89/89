...
import { assignRole, getAvailableRoles, removeRole } from '../services';
import { ApiError, sendError, sendSuccess } from '../utils';

const LOG_PREFIX = '[Role Endpoint]';

export const handleAssignRole = async (req: Request, res: Response) => {
  try {
    ...
    if (!callerFingerprintId) {
      return sendError(
        res,
        new ApiError(401, ERROR_MESSAGES.AUTHENTICATION_REQUIRED)
      );
    }
    ...
    return sendError(
      res,
      ApiError.from(error, 500, ERROR_MESSAGES.FAILED_ASSIGN_ROLE)
    );
  }
};

export const handleRemoveRole = async (req: Request, res: Response) => {
  try {
    ...
    return sendError(
      res,
      ApiError.from(error, 500, ERROR_MESSAGES.FAILED_REMOVE_ROLE)
    );
  }
};

export const handleGetAvailableRoles = async (
  _req: Request,
  res: Response
) => {
  try {
    ...
    return sendError(
      res,
      ApiError.from(error, 500, ERROR_MESSAGES.FAILED_TO_GET_AVAILABLE_ROLES)
    );
  }
};
